...
class TaskFilter {
  ...
  public static fromString(filter: string) {
    return new Database.TaskFilter(
      (filter as TaskFilterEnum) || TaskFilterEnum.all,
    );
  }
...
class TimeHorizon {
  ...
  public static fromString(timeHorizon: string) {
    return new Database.TimeHorizon(
...
export default class Database {
   ...
   */
  public static async deleteTask(userId: number, id: number): Promise<Task> {
    logger.debug(`[DATABASE] Deleting task [${id}].`);
    let deleted = await Database.getTask(id);
   ...
   * @returns A promise that resolves when the task is updated.
   */
  public static setTaskCompleted(
    userId: number,
    id: number,
    completed: boolean,
  ): Promise<Task> {
    logger.debug(
      `[DATABASE] Setting task [${id}] to completion [${completed}].`,
    );
